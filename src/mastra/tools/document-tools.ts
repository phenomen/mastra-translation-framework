...
export const readDocumentTool = createTool({
  id: 'read_document',
  description:
    'Read a document from the workspace and report its kind. Text, markdown, JSON, HTML, and subtitle documents return their content; PDFs and office/ebook documents (DOC, DOCX, EPUB, RTF, ODT) return only metadata because they must go through their conversion tools.',
  inputSchema: z.object({
    documentPath: z
      .string()
      .describe(
        'Workspace-relative path to the document (.txt, .md, .json, .html, .htm, .srt, .ass, .pdf, .doc, .docx, .epub, .rtf, .odt).',
      ),
  }),
